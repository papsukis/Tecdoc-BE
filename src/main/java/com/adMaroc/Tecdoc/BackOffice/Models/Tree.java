...
public class Tree<T> {
    ...
    public Node getNode(String s){
        return findNode(getRootElement(),s,true);
    }

    public Node<T> getNode(String s,Node<T> c){
        return findNode(c,s,false);
    }
    public Node<T> findNode(Node<T> n, String s,boolean firstsearch) {

        if (n.getData().equals(s) && firstsearch) {

            return n;

        } else {
            for (Node<T> child: n.getChildren()) {
                Node<T> result = findNode(child, s,true);
                if (result != null) {
                    return result;
                }
            }
    ...
    {
        rootElement.addChild(n);
        n.setParent(rootElement);
    }
}
